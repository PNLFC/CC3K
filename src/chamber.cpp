...
vector<int> chamber::generatePos()
{
    ...
    while (true)
    {
        currRow = rand() % rowLen;
        currCol = rand() % colLen;
        if (isValidTile(topRow + currRow, topCol + currCol))
            break;
    }
...
bool chamber::withinRange(const int row, const int col)
{
    if (id != 1 && id != 4)
    {
        if ((row >= topRow && row <= topRow + rowLen) && (col >= topCol && row <= topCol + colLen))
            return true;
        else
            return false;
    }
    else if (id == 1)
    {
        ...
        {
            if (col <= topCol + 10 && (row > topRow + 23 && row < 37))
                return true;
            else
                return false;
        }
    ...
    else
    {
        if (col <= topCol + 2)
        {
            if (row > topRow + 28 && row < topRow + 40)
                return true;
            else
                return false;
        }
        ...
        {
            if (col <= topCol + 5 && (row >= topRow && row < topRow + 40))
                return true;
            else
                return false;
        }
